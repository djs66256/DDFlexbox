...
#import "DDDynamicTableViewCell.h"

@implementation DDDynamicTableViewCellModel
- (NSString *)debugDescription {
    return [NSString stringWithFormat:@"Image: %@, Text: %@, Detail: %@", self.image == nil ? @"(null)" : @"<img>", self.text, self.detailText];
}
@end

@interface DDDynamicTableViewCell ()
...
@implementation DDDynamicTableViewCell
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        _myContentView.backgroundColor = [UIColor colorWithWhite:0 alpha:0.2];
...
- (void)updateFlexboxLayout {
    ...
    [self.flexbox setupLayout:^(id<DDFlexbox>  _Nonnull box) {
        [self.myContentView.flexbox setupLayout:^(id<DDFlexbox>  _Nonnull box) {
            ...
            [DDFlexbox.row setupLayout:^(id<DDFlexbox>  _Nonnull box) {
                ...
                if (self.model.image) {
                    // If we have image, config the image will add to superview.
                    // Otherwise the image view will not add to superview.
                ...
                [DDFlexbox.column setupLayout:^(id<DDFlexbox>  _Nonnull box) {
                    ...
                    if (self.model.text) {
                        // If we have text, config the text label will add to superview.
                        // Otherwise the text label will not add to superview.
        ...
        [self.descLabel.flexbox setupLayout:^(id<DDFlexbox>  _Nonnull box) {
            box.marginTop = YGPointValue(10);
            box.marginBottom = YGPointValue(20);
        }];
    }];
}

- (void)setModel:(DDDynamicTableViewCellModel *)model {
    _model = model;
    
    // self.myImageView.image = model.image;
    self.myTextLabel.text = model.text;
    self.myDetailTextLabel.text = model.detailText;
    
    self.descLabel.text = model.debugDescription;
    
    // We need update flexbox layout when model changed.
